...
Extras.Sync.AccordionPane = Core.extend(Echo.Render.ComponentSync, {
    ...
    _redrawTabs: function() {
        ...
        for (var i = 0; i < this._tabs.length; ++i) {
            ...
            if (selectionPassed) {
                tabDivElement.style.bottomy = "";
                tabDivElement.style.topy = (tabHeight * (this._tabs.length - i - 1)) + "px";
            } else {
                tabDivElement.style.topy = "";
                tabDivElement.style.bottomy = (tabHeight * i) + "px";
            }
            ...
            if (this._activeTabId == tab._childComponent.renderId) {
                ...
                contentDivElement.style.bottomy = (tabHeight * (i + 1)) + "px";
                var bottomPx = tabHeight * (this._tabs.length - i - 1);
                contentDivElement.style.topy = bottomPx + "px";
            } else {
    ...
    _calculateTabHeight: function() {
        ...
        return height + insets.bottomy + insets.topy + Echo.Sync.Border.getPixelSize(border) * 2;
    }
});
...
Extras.Sync.AccordionPane.Tab = Core.extend({
    ...
    _render: function(client, update) {
        ...
        this._tabDivElement.style.bottomx = "0px";
        this._tabDivElement.style.topx = "0px";
        ...
        this._contentDivElement.style.bottomx = "0px";
        this._contentDivElement.style.topx = "0px";
        ...
        if (currentTime < this._animationEndTime) {
            ...
            if (this._directionDown) {
                ...
                for (var i = 0; i < this._rotatingTabs.length; ++i) {
                    ...
                    this._rotatingTabs[i]._tabDivElement.style.bottomy = newPosition + "px";
                }

                // Adjust height of expanding new tab content to fill expanding space.
                var newContentHeight = stepPosition - this._oldTabContentInsets.bottomy - this._oldTabContentInsets.topy;
                ...
                if (this._animationStepIndex == 0) {
                    this._oldTab._contentDivElement.style.topy = "";
                    this._newTab._contentDivElement.style.display = "block";
                    this._newTab._contentDivElement.style.bottomy = (this._numberOfTabsAbove * this._tabHeight) + "px";
                }

                // Move top of old content downward.
                var oldTop = stepPosition + this._startTopPosition + (this._rotatingTabs.length * this._tabHeight);
                this._oldTab._contentDivElement.style.bottomy = oldTop + "px";
                ...
                var oldContentHeight = this._regionHeight - oldTop - ((this._numberOfTabsBelow - 1) * this._tabHeight)
                        - this._oldTabContentInsets.bottomy - this._oldTabContentInsets.topy;
                ...
                for (var i = 0; i < this._rotatingTabs.length; ++i) {
                    var newPosition = stepPosition + this._startBottomPosition
                            + (this._tabHeight * (this._rotatingTabs.length - i - 1));
                    this._rotatingTabs[i]._tabDivElement.style.topy = newPosition + "px";
                }

                // On first frame, display new tab content.
                if (this._animationStepIndex == 0) {
                    this._oldTab._contentDivElement.style.topy = "";
                    this._newTab._contentDivElement.style.bottomy = "";
                    this._newTab._contentDivElement.style.topy = (this._numberOfTabsBelow * this._tabHeight) + "px";
                ...
                var newContentHeight = stepPosition - this._newTabContentInsets.bottomy - this._newTabContentInsets.topy;
                if (newContentHeight < 0) {
                    newContentHeight = 0;
                };
                this._newTab._contentDivElement.style.height = newContentHeight + "px";
